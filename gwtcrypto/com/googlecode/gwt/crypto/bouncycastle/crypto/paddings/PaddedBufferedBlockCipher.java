package com.googlecode.gwt.crypto.bouncycastle.crypto.paddings;

import com.googlecode.gwt.crypto.bouncycastle.crypto.BlockCipher;
import com.googlecode.gwt.crypto.bouncycastle.crypto.BufferedBlockCipher;
...
import com.googlecode.gwt.crypto.bouncycastle.crypto.params.ParametersWithRandom;

/**
 * A wrapper class that allows block ciphers to be used to process data in a piecemeal fashion with
 * padding. The PaddedBufferedBlockCipher outputs a block only when the buffer is full and more data
 * is being added, or on a doFinal (unless the current block in the buffer is a pad block). The
...
public class PaddedBufferedBlockCipher extends BufferedBlockCipher {
  ...
  /**
   * return the minimum size of the output buffer required for an update plus a doFinal with an
   * input of len bytes.
   *
   * @param len
   ...
   * @return the number of output bytes copied to out.
   * @exception DataLengthException
   *              if there is insufficient space in out for the output or we are decrypting and the
   *              input is not block size aligned.
   * @exception IllegalStateException
