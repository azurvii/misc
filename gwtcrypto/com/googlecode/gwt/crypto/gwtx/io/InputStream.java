...
/**
 * The base class for all input streams. An input stream is a means of reading data from a source in
 * a byte-wise manner.
 * <p>
 * Some input streams also support marking a position in the input stream and returning to this
 * position later. This abstract class does not provide a fully working implementation, so it needs
 * to be subclassed, and at least the {@link #read()} method needs to be overridden. Overriding some
 * of the non-abstract methods is also often advised, since it might result in higher efficiency.
 * <p>
 * Many specialized input streams for purposes like reading from a file already exist in this
 * package.
 * 
 * @see OutputStream
 */
public abstract class InputStream extends Object implements Closeable {
  ...
  /**
   * Returns the number of bytes that are available before this stream will block. This
   * implementation always returns 0. Subclasses should override and indicate the correct number of
   * bytes available.
   * 
  ...
  /**
   * Reads at most {@code length} bytes from this stream and stores them in the byte array {@code b}
   * starting at {@code offset}.
   * 
   * @param b
   *          the byte array in which to store the bytes read.
   * @param offset
   *          the initial position in {@code buffer} to store the bytes read from this stream.
   * @param length
   *          the maximum number of bytes to store in {@code b}.
   * @return the number of bytes actually read or -1 if the end of the stream has been reached.
   * @throws IndexOutOfBoundsException
   *           if {@code offset < 0} or {@code length < 0}, or if {@code offset + length} is greater
   *           than the length of {@code b}.
   * @throws IOException
   *           if the stream is closed or another IOException occurs.
   */
  ...
  /**
   * Skips at most {@code n} bytes in this stream. It does nothing and returns 0 if {@code n} is
   * negative. Less than {@code n} characters are skipped if the end of this stream is reached
   * before the operation completes.
   * <p>
   * This default implementation reads {@code n} bytes into a temporary buffer. Concrete subclasses
