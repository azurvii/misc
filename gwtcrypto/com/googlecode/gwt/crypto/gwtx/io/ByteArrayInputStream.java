...
public class ByteArrayInputStream extends InputStream {
   ...
   * position set to {@code offset} and the number of bytes available set to {@code offset} +
   * {@code length}.
   * 
   * @param buf
   *          the byte array to stream over.
  ...
  /**
   * Returns the number of bytes that are available before this stream will block. This method
   * returns the number of bytes yet to be read from the source byte array.
   * 
  ...
  /**
   * Reads at most {@code len} bytes from this stream and stores them in byte array {@code b}
   * starting at {@code offset}. This implementation reads bytes from the source byte array.
   * 
   * @param b
   *          the byte array in which to store the bytes read.
   * @param offset
   *          the initial position in {@code b} to store the bytes read from this stream.
   * @param length
   *          the maximum number of bytes to store in {@code b}.
   * @return the number of bytes actually read or -1 if no bytes were read and the end of the stream
   *         was encountered.
  ...
  /**
   * Skips {@code count} number of bytes in this InputStream. Subsequent {@code read()}s will not
   * return these bytes unless {@code reset()} is used. This implementation skips {@code count}
   * number of bytes in the target stream. It does nothing and returns 0 if {@code n} is negative.
   * 
   * @param n
